...
function Sidebar() {
	...
	const { showSidebar, setShowSidebar, showAuthFormHandler, languageHandler, lang, mode, setMode } = useContext(AppContext);
	...
	return (
		<Modal>
			<nav className={` bg-[#f4f7f6] w-80 h-[100vh] absolute z-[1] ${!showSidebar && 'translate-x-[-320px]'} ease-linear duration-[300ms]`}>
				<ul>
					<li>
						<MyButton onClick={handleMode}>
							{mode === "light" ?
								<svg xmlns="http://www.w3.org/2000/svg" fill="yellow" viewBox="0 0 24 24" strokeWidth={1.5} stroke="black" className="w-6 h-6">
